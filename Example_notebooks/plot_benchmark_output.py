...
def plot_simple_regret_stuff(df_summary_1, results_dict, num_results_min, max_iteration, num_initialisation):
    ...
    for color_num, key in enumerate(keys):
        ...
        sns.lineplot(x="ids_acquired", y="max_fitness_acquired", data=df_results, ax=ax1, label=key, color=cool_colors[color_num])
        sns.lineplot(x="ids_acquired", y="mean_fitness_acquired", data=df_results, ax=ax3, color=cool_colors[color_num])
...
def plot_top_mols_acquired(results_dict, df_plot, fig, num_results_min, max_iteration, num_initialisation, top_mol_count):
    cool_colors = sns.color_palette("tab10", len(df_plot))
    keys = set(df_plot["key"].to_numpy()).intersection(set(results_dict.keys()))
    print("Keys:", keys)
    df_total_bench = pd.read_csv(df_plot["df_path"].iloc[0], low_memory=False)
    ...
    for color_num, key in enumerate(keys):
        ...
        sns.lineplot(x="ids_acquired", y="number_of_top_mol_found", data=df_results, ax=fig.axes[4], label=key, color=cool_colors[color_num])
        sns.lineplot(x="ids_acquired", y="rate_of_discovery", data=df_results, ax=fig.axes[5], color=cool_colors[color_num])
...
def main():
    ...
    fig, axes, metric_dict_res = plot_metric(
        df_all, plot_function_list_single, results_dict, nb_iterations=min_num_iteration
    )
    fig.tight_layout()
    fig.savefig("single.png")
    fig, df_plot = plot_simple_regret_stuff(df_all, results_dict, num_results_min, max_iteration, num_initialisation)
    fig.savefig("single2.png")
    ...
    plot_top_mols_acquired(results_dict, df_plot, fig_org, num_results_min, max_iteration, num_initialisation, top_mol_count)

    fig = modify_figure(fig_org, legend_list, x_limits, y_limits, tick_labels)
    fig.tight_layout()
    fig.savefig("single3.png")
