...
class Bayesian_Optimisation(Search_Algorithm):
    ...
    def suggest_element(
        ...
        if self.PCA_input:
            ...
            print("PCA Done")
        elif self.normalise_input:
            X_rpr, X_unsqueezed = self.normalise_input([X_rpr, X_unsqueezed])
        else:
            X_rpr, X_unsqueezed = X_rpr, X_unsqueezed
        # train model
    ...
    def train_test_split(self, X_explored,y_explored,test_set_size,random_state):
        def normalise_output(y):
            y_mean = y.mean()
            y_std = y.std()
            y = (y-y.min(axis=0).values)/(y.max(axis=0).values-y.min(axis=0).values)
            return y,y_mean,y_std
        np.random.seed(random_state)
        X = X_explored.cpu().type(torch.float64)
        Y = y_explored.cpu().type(torch.float64).reshape(-1,1)
        num_mols = X.shape[0]
        all_idx = np.random.permutation(num_mols)
        Ntrain = int(num_mols * test_set_size)
        train_idx = all_idx[:Ntrain]
        test_idx = all_idx[Ntrain  :]
        
     
        
        y_explored_norm, y_mean, y_std = normalise_output(Y)
        X_train = X[train_idx,:]
        X_test = X[test_idx,:]
        y_train = y_explored_norm[train_idx,:]
        y_test = y_explored_norm[test_idx,:]
    ...
    def test_model_prediction(
        self, X_train, y_train, X_test
    ):
        
        # X_train = torch.nn.functional.normalize(X_train, dim = 0)
        # X_test = torch.nn.functional.normalize(X_test, dim = 0)
        self.train_model(X_train, y_train)
        y_pred = self.model.posterior(X_test).mean
        y_var = self.model.posterior(X_test).variance
        y_pred_train = self.model.posterior(X_train).mean
        y_var_train = self.model.posterior(X_train).variance
        return (
            y_pred,
            y_var,
            y_pred_train,
    ...
    def plot_prediction(
        ...
        def plot_prediction(y_pred, y_test, axis, label):
            axis.scatter(y_test.detach().numpy(), y_pred.detach().numpy(), marker="x", color="red")
            ...
            score_mse = mean_squared_error(y_test.detach().numpy(), y_pred.detach().numpy())
            print(f"MSE score: {score_mse:.2f}")
            score_mae = mean_absolute_error(y_test.detach().numpy(), y_pred.detach().numpy())
        ...
        for x in spacing_array:
            ...
            data_plot.append(np.abs(y_pred.detach().numpy() - y_test.detach().numpy())[tes[0]].mean())
        axs[2].scatter(
            y_var,
