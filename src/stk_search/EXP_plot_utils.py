...
def plot_exp_batch_results(exp_name="Inputs/exp1_2023_09_05_14_47_02/", df_total:pd.DataFrame):
    df_list=[]
    label_list=[]
    ...
    for i in [3,4,5,6]:
        ...
        ration_top_5 =str(int(
                        df_list[-1][df_list[-1]["target"] > min_target_5percent].shape[0]
                        / df_list[-1].shape[0]
                        * 100)
                    )
        label_list.append(f'space {i}'+f'({len(df_eval)})'+    ration_top_5 + " %")
