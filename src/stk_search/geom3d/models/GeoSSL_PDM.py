...
class GeoSSL_PDM(torch.nn.Module):
    ...
    def get_score_target(pos_perturbed, pos_target, node2graph, noise_type):
        ...
        if noise_type == "riemann":
            ...
            ptp = ptp[node2graph]
            otp = otp[node2graph]
            return - 2 * (pos_perturbed_c.unsqueeze(1) @ ptp - pos_c.unsqueeze(1) @ otp).squeeze(1) / (torch.norm(ptp,dim=(1,2)) + torch.norm(otp,dim=(1,2))).unsqueeze(-1).repeat([1,3])
        else:
