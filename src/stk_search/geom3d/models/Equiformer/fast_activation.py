...
class Activation(torch.nn.Module):
    ...
    def __init__(self, irreps_in, acts):
        ...
        for (mul, (l_in, p_in)), act in zip(irreps_in, acts):
            if act is not None:
                if l_in != 0:
                    msg = "Activation: cannot apply an activation function to a non-scalar input."
                ...
                if p_out == 0:
                    msg = "Activation: the parity is violated! The input scalar is odd but the activation is neither even nor odd."
                    raise ValueError(msg)
            else:
    ...
    def extra_repr(self):
        ...
        return output_str + f"{self.irreps_in} -> {self.irreps_out}, "
...
class Gate(torch.nn.Module):
    ...
    def __init__(self, irreps_scalars, act_scalars, irreps_gates, act_gates, irreps_gated):
        ...
        if len(irreps_gates) > 0 and irreps_gates.lmax > 0:
            msg = f"Gate scalars must be scalars, instead got irreps_gates = {irreps_gates}"
            raise ValueError(msg)
        if len(irreps_scalars) > 0 and irreps_scalars.lmax > 0:
            msg = f"Scalars must be scalars, instead got irreps_scalars = {irreps_scalars}"
            raise ValueError(msg)
        if irreps_gates.num_irreps != irreps_gated.num_irreps:
            msg = f"There are {irreps_gated.num_irreps} irreps in irreps_gated, but a different number ({irreps_gates.num_irreps}) of gate scalars in irreps_gates"
            raise ValueError(msg)
