...
class EquiformerEnergyForce(torch.nn.Module):
    def __init__(self,
        ...
        max_radius=5.0,
        number_of_basis=128, basis_type="gaussian", fc_neurons=None,
    ...
    def no_weight_decay(self):
        ...
        for module_name, module in self.named_modules():
            if (isinstance(module, (torch.nn.Linear, torch.nn.LayerNorm, EquivariantLayerNormV2, EquivariantInstanceNorm, EquivariantGraphNorm, GaussianRadialBasisLayer, RadialBasis))):
    ...
    def forward(self, node_atom, pos, batch):
        ...
        return self.scale_scatter(outputs, batch, dim=0)
...
def Equiformer_nonlinear_l2_energy_force(irreps_in, radius, num_basis, node_class, **kwargs):
    return EquiformerEnergyForce(
        irreps_in=irreps_in,
        irreps_node_embedding="128x0e+64x1e+32x2e", node_class=node_class, num_layers=6,
        irreps_node_attr="1x0e", irreps_sh="1x0e+1x1e+1x2e",
...
def Equiformer_nonlinear_l2_e3_energy_force(irreps_in, radius, num_basis, node_class, **kwargs):
    return EquiformerEnergyForce(
        irreps_in=irreps_in,
...
def Equiformer_nonlinear_bessel_l2_energy_force(irreps_in, radius, num_basis, node_class, **kwargs):
    return EquiformerEnergyForce(
        irreps_in=irreps_in,
        irreps_node_embedding="128x0e+64x1e+32x2e", node_class=node_class, num_layers=6,
        irreps_node_attr="1x0e", irreps_sh="1x0e+1x1e+1x2e",
...
def Equiformer_nonlinear_exp_l2_energy_force(irreps_in, radius, num_basis, node_class, **kwargs):
    return EquiformerEnergyForce(
        irreps_in=irreps_in,
        irreps_node_embedding="128x0e+64x1e+32x2e", node_class=node_class, num_layers=6,
        irreps_node_attr="1x0e", irreps_sh="1x0e+1x1e+1x2e",
...
def Equiformer_nonlinear_exp_l3_energy_force(irreps_in, radius, num_basis, node_class, **kwargs):
    return EquiformerEnergyForce(
        irreps_in=irreps_in,
        irreps_node_embedding="128x0e+64x1e+64x2e+32x3e", node_class=node_class, num_layers=6,
...
def Equiformer_nonlinear_attn_exp_l3_energy_force(irreps_in, radius, num_basis, node_class, **kwargs):
    return EquiformerEnergyForce(
        irreps_in=irreps_in,
        irreps_node_embedding="128x0e+64x1e+64x2e+32x3e", node_class=node_class, num_layers=6,
        irreps_node_attr="1x0e", irreps_sh="1x0e+1x1e+1x2e+1x3e",
...
def Equiformer_nonlinear_exp_l3_e3_energy_force(irreps_in, radius, num_basis, node_class, **kwargs):
    return EquiformerEnergyForce(
        irreps_in=irreps_in,
        irreps_node_embedding="128x0e+64x0o+32x1e+32x1o+32x2e+32x2o+16x3e+16x3o", node_class=node_class, num_layers=6,
...
def Equiformer_nonlinear_bessel_l3_energy_force(irreps_in, radius, num_basis, node_class, **kwargs):
    return EquiformerEnergyForce(
        irreps_in=irreps_in,
        irreps_node_embedding="128x0e+64x1e+64x2e+32x3e", node_class=node_class, num_layers=6,
        irreps_node_attr="1x0e", irreps_sh="1x0e+1x1e+1x2e+1x3e",
...
def Equiformer_nonlinear_bessel_l3_e3_energy_force(irreps_in, radius, num_basis, node_class, **kwargs):
    return EquiformerEnergyForce(
        irreps_in=irreps_in,
        irreps_node_embedding="128x0e+64x0o+32x1e+32x1o+32x2e+32x2o+16x3e+16x3o", node_class=node_class, num_layers=6,
        irreps_node_attr="1x0e", irreps_sh="1x0e+1x1o+1x2e+1x3o",
