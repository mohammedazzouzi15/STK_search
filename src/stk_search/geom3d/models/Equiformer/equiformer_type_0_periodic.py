...
def get_norm_layer(norm_type):
    if norm_type == "graph":
        ...
        msg = f"Norm type {norm_type} not supported."
...
class FullyConnectedTensorProductRescaleNorm(FullyConnectedTensorProductRescale):
    ...
    def forward(self, x, y, batch, weight=None):
        ...
        return self.norm(out, batch=batch)
...
def DepthwiseTensorProduct(irreps_node_input, irreps_edge_attr, irreps_node_output,
    ...
    return TensorProductRescale(irreps_node_input, irreps_edge_attr,
...
class Vec2AttnHeads(torch.nn.Module):
    ...
    def forward(self, x):
        N, _ = x.shape
        out = []
        for _ir_idx, (start_idx, end_idx) in enumerate(self.mid_in_indices):
...
class AttnHeads2Vec(torch.nn.Module):
    ...
    def forward(self, x):
        N, _, _ = x.shape
        out = []
        for _ir_idx, (start_idx, end_idx) in enumerate(self.head_indices):
...
class GraphAttention(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if self.nonlinear_message:
            ...
            self.vec2heads_alpha = Vec2AttnHeads(o3.Irreps(f"{mul_alpha_head}x0e"),
                num_heads)
            ...
            self.vec2heads = Vec2AttnHeads(
                (o3.Irreps(f"{mul_alpha_head}x0e") + irreps_head).simplify(),
                num_heads)

        self.alpha_act = Activation(o3.Irreps(f"{mul_alpha_head}x0e"),
            [SmoothLeakyReLU(0.2)])
    ...
    def forward(self, node_input, node_attr, edge_src, edge_dst, edge_attr, edge_scalars,
        ...
        attn = value * alpha
        attn = scatter(attn, index=edge_dst, dim=0, dim_size=node_input.shape[0], reduce=REDUCE)
    ...
    def extra_repr(self):
        ...
        return output_str + f"rescale_degree={self.rescale_degree}, "
...
class NodeEmbeddingNetwork(torch.nn.Module):
    ...
    def __init__(self, irreps_node_embedding, max_atom_type, bias=True):
        ...
        self.atom_type_lin = LinearRS(o3.Irreps(f"{self.max_atom_type}x0e"),
...
class ScaledScatter(torch.nn.Module):
    ...
    def forward(self, x, index, **kwargs):
        out = scatter(x, index, reduce=REDUCE, **kwargs)
...
class EdgeDegreeEmbeddingNetwork(torch.nn.Module):
    ...
    def forward(self, node_input, edge_attr, edge_scalars, edge_src, edge_dst, batch):
        ...
        return self.scale_scatter(edge_features, edge_dst, dim=0,
...
class EquiformerEnergyPeriodic(torch.nn.Module):
    ...
    def forward_with_gathered_index(self, pos, batch, node_atom, edge_index, periodic_index_mapping, **kwargs) -> torch.Tensor:
        edge_src, edge_dst = edge_index
        edge_vec = pos.index_select(0, edge_src) - pos.index_select(0, edge_dst)
        edge_sh = o3.spherical_harmonics(l=self.irreps_edge_attr,
            x=edge_vec, normalize=True, normalization="component")
        ...
        edge_src = periodic_index_mapping[edge_src]
        edge_dst = periodic_index_mapping[edge_dst]
