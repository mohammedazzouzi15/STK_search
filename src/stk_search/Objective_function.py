...
from stk_search.Calculators import XTBcalculator
import pickle
import pandas as pd
...
class Look_up_table():
    ...
    def evaluate_element(self,element):
            ...
            #print(element)
        results =  element.merge(self.df_look_up,on=[f'InChIKey_{i}' for i in range(self.fragment_size)],how='left')
...
class IP_ES1_fosc(Objective_Function):
    def __init__(self,oligomer_size):
        ...
        self.xtb_path = "/rds/general/user/ma11115/home/anaconda3/envs/ML/bin/xtb"
        self.STDA_bin_path = '/rds/general/user/ma11115/home/bin/stda_files/xtb4stda/'
        self.Db_folder = '/rds/general/ephemeral/user/ma11115/ephemeral/BO_polymers'
    ...
    def evaluate_element(self, element): 
        ...
        db_mol = stk.MoleculeMongoDb(client, database=self.db_mol,)
        ...
        polymer = self.Build_polymer(element,
                                db=db_mol)
        polymer = self.run_xtb_opt(polymer, xtb_path, xtb_opt_output_dir,
                            database=database_new_calc, collection=collection_name+'_opt', client=client)
        Inchikey = stk.InchiKey().get_key(polymer)

        IP = self.run_xtb_ipea(polymer, xtb_path, output_dir_ipea, database=database_new_calc,
                        target='ionisation potential (eV)', collection=collection_name+'_IPEA', client=client)
        Es1 = self.run_stda(polymer, STDA_bin_path, output_dir_stda, property='Excited state energy (eV)',
                    state=1, database='stk_mohammed_BO', collection=collection_name+'_Stda', client=client)
        fosc_1 = self.run_stda(polymer, STDA_bin_path, output_dir_stda, property='Excited state oscillator strength',
                        state=1, database='stk_mohammed_BO', collection=collection_name+'_Stda', client=client)
    ...
    def run_xtb_opt(self, polymer, xtb_path, xtb_opt_output_dir, database='stk_mohammed_BO', collection='test', client=None):
    ...
    def run_xtb_ipea(self, polymer, xtb_path, xtb_opt_output_dir, database='stk_mohammed_BO', collection='testIPEA',
    ...
    def run_stda(self, polymer, STDA_bin_path, output_dir, property='Excited state energy (eV)', state=1, database='stk_mohammed', collection='test', client=None):
        collection = client[database][collection]
