...
class Plotter:
    ...
    def __init__(self, encoding_list, target, target_type, sim_type, get_desc, get_fingerprints):
        ...
        if self.__sim_type != "structural" and len(target) == 0:
            msg = "Target values missing"
            raise Exception(msg)

        # Error handeling target_type
        if len(target) > 0:
            if len(target) != len(encoding_list):
                msg = "If target is provided its length must match the instances of molecules"
                raise Exception(msg)
        ...
        if self.__sim_type == "tailored":
            ...
            if df_descriptors.empty:
                msg = "Descriptors could not be computed for given molecules"
                raise Exception(msg)
        ...
        if len(self.__mols) < 2 or len(self.__df_descriptors.columns) < 2:
            msg = "Plotter object cannot be instantiated for given molecules"
            raise Exception(msg)
    ...
    def pca(self, **kwargs):
        ...
        first_component = "PC-1 (" + f"{coverage_components[0]:.0%}" + ")"
        second_component = "PC-2 (" + f"{coverage_components[1]:.0%}" + ")"
    ...
    def tsne(self, perplexity=None, pca=False, random_state=None, **kwargs):
        ...
        if pca and self.__sim_type == "structural":
            ...
            self.__plot_title = "t-SNE plot from components with cumulative variance explained " + f"{sum(pca.explained_variance_ratio_):.0%}"
        else:
            self.__plot_title = "t-SNE plot"
        ...
        elif perplexity<5 or perplexity>50:
            pass
    ...
    def umap(self, n_neighbors=None, min_dist=None, pca=False, random_state=None, **kwargs):
        ...
        if pca and self.__sim_type == "structural":
            pca = PCA(n_components=10, random_state=random_state)
            self.__data = pca.fit_transform(self.__data)
            self.__plot_title = "UMAP plot from components with cumulative variance explained " + f"{sum(pca.explained_variance_ratio_):.0%}"
        else:
