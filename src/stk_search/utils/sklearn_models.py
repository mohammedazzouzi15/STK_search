"""Collection of functions to run sklearn models on the data."""
...
def plot_results(
    y_test, y_pred_test, y_train, y_pred_train, y_val, y_pred_val
):
...
def train_model(X_rpr, y, min_test_set=-3):
    ...
    xgb_reg = HistGradientBoostingRegressor(
        random_state=0
    )  # using 10 trees and seed=0
    # xgb_reg = GradientBoostingRegressor(n_estimators=50, random_state=0)  # using 10 trees and seed=0
    # Train and test XGBoost model
    y_pred_train, y_pred_test, y_pred_val, model = train_test_model(
        xgb_reg, X_train, y_train, X_test, y_test, X_val, y_val
    )
