...
def fragment_from_smiles(smile):
    ...
    for x in mol2.GetAtoms():
        ...
        if x.GetAtomicNum() == 6 and x.GetHybridization().name == "SP2":
            potential_connection.extend([atom.GetIdx() for atom in x.GetNeighbors() if atom.GetAtomicNum() == 1])
    mol_list = []
    mol_list_smiles=[]
...
class PrecursorDatabase(IpEs1Fosc):
    ...
    def __init__(self, client_address = "mongodb://localhost:27017/", database_name = "stk__precursor"):
    ...
    def add_precursor_to_database(self, smile, functional_groups = stk.BromoFactory):
        ...
        stk_mol = stk.BuildingBlock(smile, functional_groups=[functional_groups()])
